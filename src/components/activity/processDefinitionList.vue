	...
	<div class="fs-right">
		...
		<div class="input-box">
			<div class="input-group">
				<span class="input-group-addon">流程定义key：</span>
		...
		<el-main v-loading="fullscreenLoading" element-loading-background="rgba(255, 255, 255, 0.5)" element-loading-text="数据正在加载中" >
			<table class="table table-bordered table-activity">
				<thead>
					<tr>
						<th scope="col">id</th>
						<th scope="col">key</th>
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
                        ...
                        <td :title="item.diagramResourceName"><a @click="openDiagram(item.id,item.name)">{{item.diagramResourceName}}</a></td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'get',
							url: `${_this.baseurl}act/processDefinition/list?jqgrid`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
			...
			updateflag(itemId,name,flag){
				...
				let baseUrl=_this.baseurl+"act/processDefinition/"+(flag==0?"suspend/":"active/")+itemId;
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/act/processDefinition/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
			...
			openDiagram(id,name){
					let _this=this;
					 let layerIndex = top.layer.open({
                                    ...
                                    content: [_this.baseurl2+"static/diagram-viewer/index.html?processDefinitionId="+id],
