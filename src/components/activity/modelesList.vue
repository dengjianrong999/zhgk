	...
	<div class="fs-right">
		...
		<div class="input-box">
			...
			<div class="input-group">
                ...
                <span class="btn_content btn_query" @click="addorUpdate()">
			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
                        ...
                        <td :title="item.lastUpdateTime">{{item.lastUpdateTime}}</td>
                        <td :title="item.deploymentId">{{item.deploymentId}}</td>
                        <td :title="item.description">{{item.description}}</td>
						<td>
							<span @click="addorUpdate(item.id)"> 
								[<i class="iconfont icon-bianji"></i>]
								
							</span>
							<span @click="deployProcess(item.id)">
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'get',
							url: `${_this.baseurl}act/modeler/list?jqgrid`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/act/modeler/model/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id){
                ...
                setTimeout(() => {
					_this.$http({
							method: 'post',
							url: `${_this.baseurl}act/modeler/modeler-open2`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
                                ...
                                let layerIndex = top.layer.open({
                                    ...
                                    content: [_this.baseurl2+"static/modeler/modeler.html?modelId="+modelId],
