            ...
            _this.$http({
               ...
               if(res.status==200){
                   ...
                   for(var i in res.data){
                       ...
                       _this.fixedPoints.push({name:res.data[i].UNIT_NAME,lat:latTransform,lng:lngTrandsform,code:res.data[i].ORGANIZING_INSTITUTION_CODE,industryType:res.data[i].INDUSTRY_CATEGORY,linkman:res.data[i].LINKMAN,town:res.data[i].TOWN});
                   }
                  _this.addPoint();
               }
           })
      ...
      addPoint(){
          this.pointLayer.clearLayers();
            for (var i in this.fixedPoints) {
                ...
                }).addTo(this.pointLayer).bindPopup("<div class='markcontetn'><div class='marktables'><table class='marktable'><tbody><tr><td></td><td></td></tr><tr><td>养殖场名称：</td><td>"+this.fixedPoints[i].name+"</td></tr>"
                                                  +"<tr><td>所属镇街:</td><td>"+this.fixedPoints[i].town+"</td></tr>"
                                                  +"<tr><td>统一信用代码:</td><td>"+this.fixedPoints[i].code+"</td></tr>"
                                                  +"<tr><td>行业类别:</td><td>"+this.fixedPoints[i].industryType+"</td></tr>"
