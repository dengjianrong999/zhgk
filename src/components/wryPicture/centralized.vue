            ...
            _this.$http({
               ...
               if(res.status==200){
                   ...
                   for(var i in res.data){
                       ...
                       _this.fixedPoints.push({name:res.data[i].UNIT_NAME,lat:latTransform,lng:lngTrandsform,code:res.data[i].ORGANIZING_INSTITUTION_CODE,state:res.data[i].OPERATING_STATE,linkman:res.data[i].LINKMAN,town:res.data[i].TOWN,href:_this.baseurl3+"sjzx/zyml"});
                   }
                  _this.addPoint();
               }
           })
      ...
      addPoint(){
          this.pointLayer.clearLayers();
            for (var i in this.fixedPoints) {
                ...
                }).addTo(this.pointLayer).bindPopup("<div class='markcontetn'><div class='marktables'><table class='marktable'><tbody><tr><td></td><td></td></tr><tr><td>单位名称：</td><td>"+this.fixedPoints[i].name+"</td></tr>"
                                                  +"<tr><td>所属镇街:</td><td>"+this.fixedPoints[i].town+"</td></tr>"
                                                  +"<tr><td>统一信用代码:</td><td>"+this.fixedPoints[i].code+"</td></tr>"
                                                  +"<tr><td>运营状态:</td><td>"+this.fixedPoints[i].state+"</td></tr>"
