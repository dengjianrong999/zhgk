	...
	<div class="fs-right">
		...
		<div class="input-box">
			<div class="input-group">
				<span class="input-group-addon">排口名称：</span>
				<input type="text" class="form-control" v-model='outletName'>
			</div>
			<div class="input-group">
			...
			<div class="input-group">
				...
				<span class="btn_content btn_query" @click="openFIleForm('wrypkq')">
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th scope="col">排口名称</th>
						<th scope="col">排口ID</th>
						<th scope="col">排放口代码</th>
						<th scope="col">排口类型</th>
						<th scope="col">排气筒高度</th>
						<!-- <th scope="col">水体功能区</th>
						<th scope="col">监测因子</th> -->
                        <!-- <th scope="col">排放限值</th> -->
                        <th scope="col">评价标准</th>
                        <th scope="col">监测频率</th>
						<th scope="col">操作</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<td :title="item.OUTLET_NAME">{{item.OUTLET_NAME}}</td>
						<td :title="item.ID">{{item.ID}}</td>
						<td :title="item.OUTLET_CODE">{{item.OUTLET_CODE}}</td>
						<td :title="item.OUTLET_TYPE">{{item.OUTLET_TYPE}}</td>
						<td :title="item.OUTLET_HEIGHT">{{item.OUTLET_HEIGHT}}</td>
                        <!-- <td :title="item.codeWaterarea">{{item.codeWaterarea}}</td>
						<td :title="item.monitorFactor">{{item.monitorFactor}}</td> -->
                        <!-- <td :title="item.outLimit">{{item.outLimit}}</td> -->
						<td :title="item.NAME">{{item.NAME}}</td>
						<td :title="item.MONITOR_RATE">{{item.MONITOR_RATE}}</td>
						<td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'post',
							url: `${_this.baseurl}wrynb/outletGas/getOutletGasList`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/wrynb/outletGas/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id,num){
			   ...
			   sessionStorage.setItem('operateNum', JSON.stringify(num));
			    let layerIndex = top.layer.open({
                                    ...
                                    content: _this.baseurl2+'#/wrypkqForm/'+id ,
