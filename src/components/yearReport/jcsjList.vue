	...
	<div class="fs-right">
		...
		<div class="input-box">
			...
			<div class="input-group">
				...
				<span class="btn_content btn_query" @click="openFIleForm('jcsjs')">
			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<!-- <td :title="item.IS_STANDARD">{{item.IS_STANDARD}}</td> -->
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'post',
							url: `${_this.baseurl}wrynb/MonitorWater/getMonitorWaterList`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/wrynb/MonitorWater/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id,num){
			   ...
			   sessionStorage.setItem('operateNum', JSON.stringify(num));
			    let layerIndex = top.layer.open({
                                    ...
                                    content: _this.baseurl2+'#/jcsjForm/'+id ,
