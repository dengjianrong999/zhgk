	...
	<div class="fs-right">
		...
		<div class="input-box">
			<div class="input-group">
				<span class="input-group-addon">因子名称：</span>
				<input type="text" class="form-control" v-model='modelname'>
			</div>
			
			<div class="input-group">
				<span class="input-group-addon">标准名称：</span>
				<input type="text" class="form-control" v-model='standardname'>
			</div>
			<!--按钮-->
			<div class="input-group">
				...
				<span class="btn_content btn_query" @click="openFIleForm('pjxms')">
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th scope="col">单位</th>
						<th scope="col">是否特征污染物</th>
						<!-- <th scope="col">评价标准</th> -->
						<!-- <th scope="col">水体执行标准</th> -->
						<th scope="col">评价标准</th>
						<th scope="col">操作</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<td :title="item.LIMIT_VALUE">{{item.LIMIT_VALUE}}</td>
						<td :title="item.UNIT">{{item.UNIT}}</td>
                        <td :title="item.IS_TZXFACTORS">{{item.IS_TZXFACTORS}}</td>
						<!-- <td :title="item.standardId">{{item.standardId}}</td> -->
						<!-- <td :title="item.EXECUTIVE_STANDARD">{{item.EXECUTIVE_STANDARD}}</td> -->
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'post',
							url: `${_this.baseurl}wrynb/MonitorWater/getLimitWaterList`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/wrynb/LimitFactorW/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id,num){
			   ...
			   sessionStorage.setItem('operateNum', JSON.stringify(num));
			    let layerIndex = top.layer.open({
                                    ...
                                    content: _this.baseurl2+'#/pjxmsForm/'+id ,
