	...
	<div class="fs-right">
		...
		<div class="input-box">
			...
			<div class="input-group">
				...
				<span class="btn_content btn_query" @click="openFIleForm('jcsjq')">
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th scope="col">监测值</th>
						<th scope="col">监测类别</th>
                        <th scope="col">应用类别</th>
						<th scope="col">类别</th>
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
                        ...
                        <td :title="item.MONITOR_VALUE">{{item.MONITOR_VALUE}}</td>
						<td :title="item.MONITOR_TYPE">{{item.MONITOR_TYPE}}</td>
						<td :title="item.USE_TYPES">{{item.USE_TYPES}}</td>
						<td :title="item.TYPES">{{item.TYPES}}</td>
                        <td :title="item.MONITOR_TIME">{{item.MONITOR_TIME.substring(0,10)}}</td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'post',
							url: `${_this.baseurl}wrynb/MonitorGas/getMonitorGasList`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/wrynb/MonitorGas/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id,num){
			   ...
			   sessionStorage.setItem('operateNum', JSON.stringify(num));
			    let layerIndex = top.layer.open({
                                    ...
                                    content: _this.baseurl2+'#/jcsjqForm/'+id ,
