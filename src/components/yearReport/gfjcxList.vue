	...
	<div class="fs-right">
		...
		<div class="input-box">
			<div class="input-group">
				<span class="input-group-addon">监测因子：</span>
				<input type="text" class="form-control" v-model='monitorFactor'>
			</div>
			...
			<div class="input-group">
				...
				<span class="btn_content btn_query" @click="openFIleForm('gfjcx')">
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						<th scope="col">序号</th>
						<th scope="col">类型</th>
						<th scope="col">监测分类</th>
						<th scope="col">监测因子</th>
						<th scope="col">执行标准</th>
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<td :title="item.monitorType">{{item.monitorType}}</td>
						<td :title="item.monitorFactor">{{item.monitorFactor}}</td>
                        <td :title="item.standardId">{{item.standardId}}</td>
						<td :title="item.limitVal">{{item.limitVal}}</td>
	...
	export default {
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				let _this = this;
				let temData = {
					monitorFactor:this.monitorFactor,
					page: this.page,
					rows: this.rows
				};
				setTimeout(() => {
					_this.$http({
							method: 'get',
							url: `${_this.baseurl}wrynb/MonitorItems/listJgrid`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/wrynb/MonitorItems/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id,num){
			   ...
			   sessionStorage.setItem('operateNum', JSON.stringify(num));
			    let layerIndex = top.layer.open({
                                    ...
                                    content: _this.baseurl2+'#/gfjcxForm/'+id ,
