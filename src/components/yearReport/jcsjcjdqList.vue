	...
	<div class="fs-right">
		...
		<div class="input-box">
			...
			<div class="input-group">
				...
				<span class="btn_content btn_query" @click="openFIleForm('gfjcsjcjdq')">
			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<td :title="item.CHECKPOINT">{{item.CHECKPOINT}}</td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'post',
							url: `${_this.baseurl}wrynb/gfFborder/getGfFactorBorderList`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/wrynb/gfFborder/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id,num){
			   ...
			   sessionStorage.setItem('operateNum', JSON.stringify(num));
			    let layerIndex = top.layer.open({
                                    ...
                                    content: _this.baseurl2+'#/jcsjcjdqForm/'+id ,
