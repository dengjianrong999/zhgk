       ...
       <div class="block">
        ...
        <div class="block">
            ...
            <div class="value">
               ...
               <el-select filterable placeholder="--请选择--"  v-model="outletId" :disabled="disabledFlag"  @change="getFactorName()">
        ...
        <div class="block">
            ...
            <div class="value">
			   <!-- <input class="myinput" v-model="useTypes" > -->
               <el-select filterable placeholder="--请选择--"  v-model="useTypes" :disabled="disabledFlag">
					<el-option v-for="item in useTypesList" :key="item.value" :label="item.text" :value="item.value">
					</el-option>
				</el-select>
            </div>
        </div>
        <div class="block">
            ...
            <div class="value">
			   <input class="myinput" v-model="tyDate" id="tyDate">
    ...
    export default {
        data() {
            return {
             ...
             id:"",
             types:"",
             typesList:[{value:"VOC"},{value:"锅炉废气"},{value:"重金属废气"}],
             useTypes:"",
             useTypesList:[{value:"折算"},{value:"实测"},{value:"燃料消耗量"},{value:"其他"}],
            factorId:"",
            factorName:"",
            monitorType:"监督性监测",
        ...
        methods: {
            ...
            saves() {
                ...
                if(this.factorId=="" || this.monitorType=="" || this.monitorTime=="" || this.types=="" || this.outletId=="" || this.useTypes==""){
                    layer.msg("请填完必填项");
                }else{
                ...
                this.$http({
                            method: 'post',
                            url: `${this.baseurl}/wrynb/MonitorGas/save`,
                            data:paramData
                        })
                        .then(function (res) {
                            if (res.status === 200) {
                 ...
                 let _this = this;
                  let operateNum = JSON.parse(sessionStorage.getItem('operateNum')); // 操作类型 0详情 1修改
                    ...
                    if(id!='111'){
                        this.$http({
                            method: 'post',
                            url: _this.baseurl + "/wrynb/MonitorGas/data/"+id ,
                        ...
                        .then(function (res) {
                         ...
                         _this.useTypes=res.data.data.useTypes;
                         if(res.data.data.tyDate!=null && res.data.data.tyDate!=""){
                              _this.tyDate=res.data.data.tyDate;
                         }
                        // if(res.data.data.outletId!=null && res.data.data.outletId!=""){
                        //     _this.$http({
                        //     url:_this.baseurl+"wrynb/MonitorGas/getFactorNamebyOutId?outletId="+res.data.data.outletId,
                        //     method:'post'
                        // }).then(function(res2){
                        //     _this.nameList=[];
                        ...
                        if(res.data.data.factorId!=null && res.data.data.factorId!=''){
                            _this.$http({
                                url:_this.baseurl+"wrynb/LimitFactorG/data/"+res.data.data.factorId,
                                method:'get'
                            }).then(function (res2){
                    ...
                    // this.$http({
                    //     method:'get',
                    //     url:_this.baseurl+"wrynb/LimitFactorG/listJson"
                    // }).then(function(res){
                        
                    //     for(var i in res.data.data){
                    //         _this.nameList.push({value:res.data.data[i].id,text:res.data.data[i].name});
                    //     }
                    // });
                    this.$http({
                        method:'post',
                        url:_this.baseurl+"wrynb/outletGas/getOutletGasSelectList"
                    }).then(function(res){
                        for(var i in res.data){
                            _this.outletList.push({value:res.data[i].ID,text:res.data[i].OUTLET_NAME+'('+res.data[i].COMPANY_NAME+'('+res.data[i].YEARS+')'+')'});
                        }
            ...
            getFactorName(){
                let _this=this;
                _this.$http({
                    url:_this.baseurl+"wrynb/MonitorGas/getFactorNamebyOutId?outletId="+_this.outletId,
                    method:'post'
                }).then(function(res){
                    _this.nameList=[];
                    for(var i in res.data){
