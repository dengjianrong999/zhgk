	...
	<div class="fs-right">
		...
		<div class="input-box">
			<div class="input-group">
				<span class="input-group-addon">测点名称：</span>
				<input type="text" class="form-control" v-model='checkpoint'>
			</div>
			...
			<div class="input-group">
				...
				<span class="btn_content btn_query" @click="openFIleForm('gfjcsjstyAndDxs')">
		...
		<el-main v-loading="fullscreenLoading" element-loading-background="rgba(255, 255, 255, 0.5)" element-loading-text="数据正在加载中" >
			<table class="table table-bordered">
				<thead>
					<tr>
						<th scope="col">序号</th>
						<th scope="col">填埋场名称</th>
						<th scope="col">测点名称</th>
						<th scope="col">测点代码</th>
						<th scope="col">渗透液排放量（吨/天）</th>
                        <th scope="col">监测因子</th>
						<th scope="col">监测值</th>
                        <th scope="col">监测日期</th>
						<th scope="col">操作</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<td :title="item.STYNAME">{{item.STYNAME}}</td>
						<td :title="item.STYCODE">{{item.STYCODE}}</td>
                        <td :title="item.STY_EMISSIONS">{{item.STY_EMISSIONS}}</td>
						<td :title="item.NAME">{{item.NAME}}</td>
                        <td :title="item.MONITOR_VAL">{{item.MONITOR_VAL}}</td>
                        <td :title="item.MONITOR_DATE">{{item.MONITOR_DATE.substring(0,10)}}</td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'post',
							url: `${_this.baseurl}wrynb/gfPentrant/getGfStyList`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/wrynb/gfPentrant/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id,num){
			   ...
			   sessionStorage.setItem('operateNum', JSON.stringify(num));
			    let layerIndex = top.layer.open({
                                    ...
                                    content: _this.baseurl2+'#/gfstyForm/'+id ,
