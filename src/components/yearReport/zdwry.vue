	...
	<div class="fs-right">
		...
		<div class="input-box">
			...
			<div class="input-group">
				...
				<input type="text" class="form-control" v-model='years' id='years'>
			</div>
			<!-- <div class="input-group">
			</div> -->
			<!--按钮-->
			<div class="input-group">
				...
				<span class="btn_content btn_query" @click="openFIleForm('zdwry')">
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th scope="col">污染源类型</th>
						<!-- <th scope="col">处理工艺</th>
						<th scope="col">处理量（日/设计）</th>
						<th scope="col">处理量（日/实际）</th> -->
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<td :title="item.TYPE_ID">{{item.TYPE_ID}}</td>
					
						<td :title="item.INDUSTRY_TYPE">{{item.INDUSTRYNAME}}</td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'POST',
							url: `${_this.baseurl}wrynb/company/getCompanyList`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/wrynb/company/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id,num){
			   ...
			   sessionStorage.setItem('operateNum', JSON.stringify(num));
			    let layerIndex = top.layer.open({
                                    ...
                                    content: _this.baseurl2+'#/zdwryForm/'+id ,
