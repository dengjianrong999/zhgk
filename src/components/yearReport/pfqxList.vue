	...
	<div class="fs-right">
		...
		<div class="input-box">
			<div class="input-group">
				<span class="input-group-addon">水体类别：</span>
			...
			<div class="input-group">
				...
				<span class="btn_content btn_query" @click="openFIleForm('pfqx')">
		...
		<el-main v-loading="fullscreenLoading" element-loading-background="rgba(255, 255, 255, 0.5)" element-loading-text="数据正在加载中" >
			<table class="table table-bordered">
				<thead>
					<tr>
						<th scope="col">序号</th>
						<th scope="col">水体类别</th>
						<!-- <th scope="col">执行标准</th> -->
						<th scope="col">水体名称</th>
						<!-- <th scope="col">流域类型</th> -->
						<!-- <th scope="col">联审标准</th> -->
						<th scope="col">操作</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<!-- <td :title="item.executStandard">{{item.executStandard}}</td> -->
                        <td :title="item.names">{{item.names}}</td>
						<!-- <td :title="item.riverType">{{item.riverType}}</td> -->
                         <!-- <td :title="item.checkStandard">{{item.checkStandard}}</td> -->
						<td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				setTimeout(() => {
					_this.$http({
							method: 'get',
							url: `${_this.baseurl}wrynb/OutwhereWater/listJgrid`,
							params: temData
						})
						.then(function (res) {
							if (res.status == 200 ) {
				...
				layer.confirm(
					...
					function (index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "/wrynb/OutwhereWater/delete/" + id
							})
							.then(function (res) {
								_this.getpage();
							})
            ...
            addorUpdate(id,num){
			   ...
			   sessionStorage.setItem('operateNum', JSON.stringify(num));
			    let layerIndex = top.layer.open({
                                    ...
                                    content: _this.baseurl2+'#/pfqxForm/'+id ,
